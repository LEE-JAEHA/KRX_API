...
#회사 리스트 받아옴
file = open("./cp_list","rb")
cp_list = pickle.load(file)
...
for i,v in enumerate(y_valid):
    if i%100 == 0 and i>1:
        good_result.append(i//100)
        bad_result.append(i//100)
    if v == True : # 실제 오른 경우
        uu+=1
        if v == y_pred[i] :#실제 올랐는데 올랐다고 판단
            good_result.append(cp_list[i%100])
            up_up+=1
    ...
    if v== False: #실제 떨어졌는데
        ...
        if v != y_pred[i]:  # 실제 내렸는데 올랐다고 판단
            bad_result.append(cp_list[i%100])
            down_up += 1
print("*"*30)
print("실제 오른 것 개수 : ",uu)
