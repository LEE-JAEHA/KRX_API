...
train_pre = train_pre.iloc[:,:-2]#이부분을 통해 indicator 어떤 것들을 넣을 것인지
len_ = len(train_pre)-1000 # len_ 는 얼마나 train에 참여할지 . 여기서는 20200406에서 10일 전까지
x_valid = train_pre.iloc[len_:]
y_valid = tmp.iloc[len_:]
train_pre = train_pre.iloc[0:len_-100]
up_list = tmp.iloc[100:len_]
...
tree_clf = DecisionTreeClassifier(criterion='entropy',max_depth=8 , random_state=13)
...
export_graphviz(tree_clf, out_file="indicator_DT.dot",feature_names=menu_list[6:-2],class_names=["UP","DOWN"], filled=True, impurity=False)
